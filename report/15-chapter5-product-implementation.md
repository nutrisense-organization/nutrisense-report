...
| US44 | Connect Google Fit Account | T109 | Define WearableConnection entity and value objects | Define the WearableConnection entity (id, userId, provider, status, connectedAt) and WearableProvider (GOOGLE_FIT) and WearableStatus (CONNECTED, DISCONNECTED, ERROR) value objects. | 2 | Mora Rivera, Joel Fernando | Done |
| US44 | Connect Google Fit Account | T110 | Scaffold the Google Fit integration | Define the data models needed and set up the skeleton connection to Google Fit, including the OAuth flow so the user can link their account. The connection screen should be in place even though full sync is still pending. | 3 | Mora Rivera, Joel Fernando | Done |
| US44 | Connect Google Fit Account | T111 | Implement OAuth redirect and token exchange | Build the OAuth initiation (redirect to Google consent screen) and callback handler (exchange authorization code for token, store connection state). | 3 | Mora Rivera, Joel Fernando | Done |
| US44 | Connect Google Fit Account | T112 | Handle OAuth failure and timeout gracefully | Show a clear error message if the user denies the OAuth consent or if the token exchange times out, and allow them to retry. | 2 | Mora Rivera, Joel Fernando | Done |
| US44 | Connect Google Fit Account | T113 | Integrate connection state with Activity Pinia store | Wire the connection screen to the store's `connectWearable` and `fetchWearableStatus` actions and display current connection status reactively. | 1 | Mora Rivera, Joel Fernando | Done |
| US44 | Connect Google Fit Account | T114 | Functional verification of OAuth initiation and status update | Verify that clicking "Connect Google Fit" initiates the OAuth flow and that the connection status badge updates to CONNECTED after success. | 1 | Mora Rivera, Joel Fernando | Done |
...
| US48 | Export Progress Report as PDF | T163 | Define ProgressReport view model | Define a ProgressReport view model that composes body metrics snapshots, weekly nutrition averages, and physical activity totals for a user-selected date range. | 2 | Villarreal Bazan, Angel Martin | Done |
| US48 | Export Progress Report as PDF | T164 | Register the export route and scaffold PDF generation | Register the export route in the Vue router and wire up the PDF generation logic. The report should summarize the chosen period: body metrics, weekly nutrition, and physical activity. The download button should be visible from the dashboard. | 4 | Villarreal Bazan, Angel Martin | Done |
| US48 | Export Progress Report as PDF | T165 | Build the report layout component with period selector | Create a report-preview component with a date range picker and content sections for body metrics, nutrition, and activity. | 3 | Villarreal Bazan, Angel Martin | Done |
| US48 | Export Progress Report as PDF | T166 | Validate that PDF is not generated for empty periods | Show an informative message and disable the download button if no data exists for the selected date range. | 2 | Villarreal Bazan, Angel Martin | Done |
| US48 | Export Progress Report as PDF | T167 | Integrate report data with Analytics Pinia store | Wire the period selector to the store's `fetchReportData` action and pass the assembled view model to the PDF generation service. | 1 | Villarreal Bazan, Angel Martin | Done |
| US48 | Export Progress Report as PDF | T168 | Functional verification of PDF export | Verify that the downloaded PDF contains correct body metrics, nutrition summary, and activity data for the selected period. | 1 | Villarreal Bazan, Angel Martin | Done |
...
| TS23 | API: Cross-Cutting IDOR Ownership Validation | T129 | Design reusable ownership-validation guard | Design and implement a reusable guard clause (the `GetAuthenticatedUserId()` controller extension) that compares route {userId}/{id} against the JWT sub claim. | 6 | Villarreal Bazan, Angel Martin | Done |
| TS23 | API: Cross-Cutting IDOR Ownership Validation | T130 | Apply IDOR filter to IAM, Subscriptions & SmartRecommendations | Apply the ownership-validation filter to users, sessions, subscriptions, pantry, recommendations, recipes and location-preferences endpoints. | 4 | Villarreal Bazan, Angel Martin | Done |
| TS23 | API: Cross-Cutting IDOR Ownership Validation | T131 | Apply IDOR filter to Nutrition Tracking | Apply the ownership-validation filter to nutrition-logs endpoints. | 3 | Mora Rivera, Joel Fernando | Done |
| TS23 | API: Cross-Cutting IDOR Ownership Validation | T132 | Apply IDOR filter to Body Health Metrics | Apply the ownership-validation filter to body-metrics endpoints. | 3 | Vergraray Calderon, Rose Almendra | Done |
| TS23 | API: Cross-Cutting IDOR Ownership Validation | T133 | Apply IDOR filter to Activity & Wearable | Apply the ownership-validation filter to activity-logs and wearable-connections endpoints. | 3 | Espinoza Cruz, Angela Milagros | Done |
| TS23 | API: Cross-Cutting IDOR Ownership Validation | T134 | Apply IDOR filter to Analytics & Reporting | Apply the ownership-validation filter to analytics endpoints. | 3 | Del Aguila Del Aguila, Olenka Priscilla | Done |
...
Durante el Sprint 3, el equipo completó la implementación y despliegue del backend de NutriSense (`Nutrisense.Platform`) en **.NET 10 / C#** bajo una arquitectura DDD + Clean Architecture distribuida en siete bounded contexts: IAM, BodyHealthMetrics, NutritionTracking, ActivityWearable, SmartRecommendations, Subscriptions & Billing y AnalyticsReporting. Se entregaron los 78 endpoints REST versionados bajo `api/v1/`, integrando servicios externos de inteligencia artificial (Gemini, DeepSeek), fuentes de datos nutricionales (USDA FoodData Central), clima (OpenWeatherMap) y pagos (Stripe). La totalidad de los endpoints fue documentada con **OpenAPI / Swagger UI** y desplegada en producción. Adicionalmente, se realizó la integración completa con el frontend Vue del Sprint 2, que migró de `json-server` a los endpoints reales, habilitando flujos end-to-end: onboarding con saga de cálculo IMC → BMR → TDEE → meta diaria, escaneo de plato con visión por IA, log nutricional persistido, activación de suscripción vía Stripe y generación de recomendaciones contextualizadas con restricciones dietéticas y clima en tiempo real. El Sprint Goal se cumplió al 100 %.

A continuación se presentan screenshots de las principales vistas implementadas y desplegadas durante el sprint.

**Swagger UI — Documentación completa de los 78 endpoints REST**
...
La documentación OpenAPI/Swagger generada automáticamente por el backend fue habilitada en el entorno de producción, permitiendo explorar y probar los 78 endpoints REST desde el navegador sin configuración adicional.
...
| US ID | US Title | Task ID | Task Title | Description | Est. (h) | Assigned To | Status |
|-------|----------|---------|------------|-------------|----------|-------------|--------|
| TS24 | API: Cross-Cutting Error Handling Foundation with RFC 7807 ProblemDetails | T1 | Implement shared ProblemDetails factory (RFC 7807) | Implement a shared factory that maps consolidated domain errors to `ProblemDetails` responses following RFC 7807. | 4 | Espinoza Cruz, Angela Milagros | Done |
| TS24 | API: Cross-Cutting Error Handling Foundation with RFC 7807 ProblemDetails | T2 | Add controller base extensions for standardized responses | Add base controller extensions that expose helpers to return `ProblemDetails` and localized messages uniformly across contexts. | 3 | Espinoza Cruz, Angela Milagros | Done |
| TS24 | API: Cross-Cutting Error Handling Foundation with RFC 7807 ProblemDetails | T3 | Sync EF model snapshot and background seeding host | Sync the EF model snapshot and align the background seeding host with the consolidated model changes. | 2 | Espinoza Cruz, Angela Milagros | Done |
| TS24 | API: Cross-Cutting Error Handling Foundation with RFC 7807 ProblemDetails | T4 | Wire new services into startup | Register the ProblemDetails factory, SMTP email service, Stripe gateway and Google Health provider into the application startup pipeline. | 3 | Espinoza Cruz, Angela Milagros | Done |
| TS24 | API: Cross-Cutting Error Handling Foundation with RFC 7807 ProblemDetails | T5 | Update deployment configuration | Update the deployment configuration and environment variables for the new external integrations. | 2 | Espinoza Cruz, Angela Milagros | Done |
| TS24 | API: Cross-Cutting Error Handling Foundation with RFC 7807 ProblemDetails | T6 | Bump platform version and verify integration | Bump the platform version and validate that all contexts return standardized `ProblemDetails` responses end-to-end. | 1 | Espinoza Cruz, Angela Milagros | Done |
| TS25 | API: Password Recovery Flow and Localized IAM Error Responses | T7 | Define PasswordResetToken aggregate and consolidated IamDomainError | Define the `PasswordResetToken` aggregate and consolidate the per-command IAM errors into a single `IamDomainError`. | 4 | Villarreal Bazan, Angel Martin | Done |
| TS25 | API: Password Recovery Flow and Localized IAM Error Responses | T8 | Define reset contracts and add request/reset password commands | Define the password reset repository and email service contracts and add the request-password and reset-password commands. | 3 | Villarreal Bazan, Angel Martin | Done |
| TS25 | API: Password Recovery Flow and Localized IAM Error Responses | T9 | Implement password reset token persistence and EF migration | Implement persistence for the password reset token and add the EF migration for the password reset tokens table. | 4 | Villarreal Bazan, Angel Martin | Done |
| TS25 | API: Password Recovery Flow and Localized IAM Error Responses | T10 | Implement SMTP email service and wire the reset flow | Implement the SMTP email service (MailKit) and wire the password reset flow into the user command service and aggregate. | 5 | Villarreal Bazan, Angel Martin | Done |
| TS25 | API: Password Recovery Flow and Localized IAM Error Responses | T11 | Add forgot/reset REST resources and unified action result assembler | Add the forgot/reset password REST resources and replace the per-command result assemblers with a single action result assembler. | 4 | Villarreal Bazan, Angel Martin | Done |
| TS25 | API: Password Recovery Flow and Localized IAM Error Responses | T12 | Return ProblemDetails and add localized IAM messages | Return `ProblemDetails` with authenticated-user checks from the REST layer and add localized (ES/EN) messages for password reset. | 3 | Villarreal Bazan, Angel Martin | Done |
| TS26 | API: Body Health Metrics Localized Error Responses | T13 | Define consolidated BodyHealthMetricsDomainError | Consolidate the per-command errors of the context into a single domain error. | 2 | Espinoza Cruz, Angela Milagros | Done |
| TS26 | API: Body Health Metrics Localized Error Responses | T14 | Implement action result assembler for the consolidated error | Implement a single action result assembler mapping the consolidated domain error to REST responses. | 3 | Espinoza Cruz, Angela Milagros | Done |
| TS26 | API: Body Health Metrics Localized Error Responses | T15 | Remove per-command application errors | Remove the per-command application errors superseded by the consolidated domain error. | 2 | Espinoza Cruz, Angela Milagros | Done |
| TS26 | API: Body Health Metrics Localized Error Responses | T16 | Return ProblemDetails from the REST layer | Refactor the REST layer to return `ProblemDetails` responses for all body-metrics endpoints. | 3 | Espinoza Cruz, Angela Milagros | Done |
| TS26 | API: Body Health Metrics Localized Error Responses | T17 | Add localized messages (ES/EN) | Add localized (ES/EN) `.resx` messages for the consolidated body-metrics errors. | 2 | Espinoza Cruz, Angela Milagros | Done |
| TS27 | API: Nutrition Tracking Localized Error Responses | T18 | Define consolidated NutritionTrackingDomainError and assembler | Consolidate the per-command errors into a single domain error and add the corresponding action result assembler. | 3 | Espinoza Cruz, Angela Milagros | Done |
| TS27 | API: Nutrition Tracking Localized Error Responses | T19 | Remove per-command application errors | Remove the per-command application errors superseded by the consolidated domain error. | 2 | Espinoza Cruz, Angela Milagros | Done |
| TS27 | API: Nutrition Tracking Localized Error Responses | T20 | Replace per-command result assemblers with a single one | Replace the per-command result assemblers with a single unified action result assembler. | 3 | Espinoza Cruz, Angela Milagros | Done |
| TS27 | API: Nutrition Tracking Localized Error Responses | T21 | Return ProblemDetails from the REST layer | Refactor the REST layer to return `ProblemDetails` responses for all nutrition-tracking endpoints. | 3 | Espinoza Cruz, Angela Milagros | Done |
| TS27 | API: Nutrition Tracking Localized Error Responses | T22 | Add localized messages (ES/EN) | Add localized (ES/EN) `.resx` messages for the consolidated nutrition-tracking errors. | 2 | Espinoza Cruz, Angela Milagros | Done |
| TS28 | API: Google Health Auto-Sync and Localized Activity Error Responses | T23 | Add consolidated ActivityWearableDomainError and remove per-command errors | Consolidate the per-command errors into a single domain error and remove the superseded application errors. | 3 | Del Aguila Del Aguila, Olenka Priscilla | Done |
| TS28 | API: Google Health Auto-Sync and Localized Activity Error Responses | T24 | Add active calorie estimator and Google Health provider | Add the MET-based active calorie estimator contract and integrate the Google Health sync provider. | 5 | Del Aguila Del Aguila, Olenka Priscilla | Done |
| TS28 | API: Google Health Auto-Sync and Localized Activity Error Responses | T25 | Add set auto-sync command and persist preference with EF migration | Add the set auto-sync command, persist the auto-sync preference on the wearable connection and add the EF migration for health sync settings. | 4 | Del Aguila Del Aguila, Olenka Priscilla | Done |
| TS28 | API: Google Health Auto-Sync and Localized Activity Error Responses | T26 | Wire auto-sync and health estimation and add REST resource | Wire auto-sync and health estimation into the command service and add the set auto-sync REST resource. | 4 | Del Aguila Del Aguila, Olenka Priscilla | Done |
| TS28 | API: Google Health Auto-Sync and Localized Activity Error Responses | T27 | Replace per-command result assemblers with a unified assembler | Replace the per-command result assemblers with a single action result assembler. | 3 | Del Aguila Del Aguila, Olenka Priscilla | Done |
| TS28 | API: Google Health Auto-Sync and Localized Activity Error Responses | T28 | Return ProblemDetails and add localized messages (ES/EN) | Return `ProblemDetails` from the REST layer and add localized messages for health sync and consolidated errors. | 3 | Del Aguila Del Aguila, Olenka Priscilla | Done |
| TS29 | API: Location-Aware Recommendations with Localized Error Responses | T29 | Add consolidated SmartRecommendationsDomainError and remove per-command errors | Consolidate the per-command errors into a single domain error and remove the superseded application errors. | 3 | Vergraray Calderon, Rose Almendra | Done |
| TS29 | API: Location-Aware Recommendations with Localized Error Responses | T30 | Add set location permission command | Add the set location permission command to the context application layer. | 3 | Vergraray Calderon, Rose Almendra | Done |
| TS29 | API: Location-Aware Recommendations with Localized Error Responses | T31 | Wire location permission into recipe improvement | Wire the location permission into the recipe improvement/recommendation flow. | 4 | Vergraray Calderon, Rose Almendra | Done |
| TS29 | API: Location-Aware Recommendations with Localized Error Responses | T32 | Add set location permission REST resource | Add the set location permission REST resource and endpoint. | 3 | Vergraray Calderon, Rose Almendra | Done |
| TS29 | API: Location-Aware Recommendations with Localized Error Responses | T33 | Add action result assembler and return ProblemDetails | Add the action result assembler for the consolidated error and return `ProblemDetails` from the REST layer. | 3 | Vergraray Calderon, Rose Almendra | Done |
| TS29 | API: Location-Aware Recommendations with Localized Error Responses | T34 | Add localized messages (ES/EN) for location permission | Add localized (ES/EN) messages for location permission and the consolidated errors. | 2 | Vergraray Calderon, Rose Almendra | Done |
| TS30 | API: Stripe Payment Gateway and Localized Subscription Error Responses | T35 | Add consolidated SubscriptionsDomainError and remove per-command errors | Consolidate the per-command errors into a single domain error and remove the superseded application errors. | 3 | Mora Rivera, Joel Fernando | Done |
| TS30 | API: Stripe Payment Gateway and Localized Subscription Error Responses | T36 | Store Stripe customer id with EF migration | Store the Stripe customer id on user subscriptions and add the EF migration for the new column. | 3 | Mora Rivera, Joel Fernando | Done |
| TS30 | API: Stripe Payment Gateway and Localized Subscription Error Responses | T37 | Integrate Stripe payment gateway | Integrate the Stripe payment gateway and reference the Stripe SDK package. | 5 | Mora Rivera, Joel Fernando | Done |
| TS30 | API: Stripe Payment Gateway and Localized Subscription Error Responses | T38 | Wire Stripe billing into command services | Wire the Stripe billing into the subscription command services for real payment processing. | 4 | Mora Rivera, Joel Fernando | Done |
| TS30 | API: Stripe Payment Gateway and Localized Subscription Error Responses | T39 | Add action result assembler and return ProblemDetails | Add the action result assembler for the consolidated error and return `ProblemDetails` from the REST layer. | 3 | Mora Rivera, Joel Fernando | Done |
| TS30 | API: Stripe Payment Gateway and Localized Subscription Error Responses | T40 | Add localized subscriptions messages (ES/EN) | Add the localized (ES/EN) subscriptions message resources. | 2 | Mora Rivera, Joel Fernando | Done |
| TS31 | API: Analytics Reporting Localized Error Responses | T41 | Define consolidated AnalyticsReportingDomainError and assembler | Consolidate the per-command errors into a single domain error and add the corresponding action result assembler. | 3 | Espinoza Cruz, Angela Milagros | Done |
| TS31 | API: Analytics Reporting Localized Error Responses | T42 | Remove per-command application errors | Remove the per-command application errors superseded by the consolidated domain error. | 2 | Espinoza Cruz, Angela Milagros | Done |
| TS31 | API: Analytics Reporting Localized Error Responses | T43 | Return ProblemDetails from the REST layer | Refactor the REST layer to return `ProblemDetails` responses for all analytics endpoints. | 3 | Espinoza Cruz, Angela Milagros | Done |
| TS31 | API: Analytics Reporting Localized Error Responses | T44 | Add localized messages (ES/EN) | Add localized (ES/EN) `.resx` messages for the consolidated analytics errors. | 2 | Espinoza Cruz, Angela Milagros | Done |
| TS31 | API: Analytics Reporting Localized Error Responses | T45 | Verify standardized error responses | Validate that all analytics-reporting endpoints return standardized localized `ProblemDetails` responses. | 2 | Espinoza Cruz, Angela Milagros | Done |

#### 5.2.4.4. Development Evidence for Sprint Review

Durante el Sprint 4, el equipo concentró la implementación en el Web Service (`nutrisense-platform`), estandarizando de forma transversal el manejo de errores de los siete bounded contexts y completando las integraciones externas pendientes, y en la Web Application (`nutrisense-webapp`), donde se consumieron esas nuevas capacidades desde la interfaz. El backend se liberó de forma incremental hacia `develop` y `main` con versionado semántico (de `v1.2.0` hasta el release final `v2.1.0`). En cada bounded context se aplicó el mismo patrón: consolidación de los errores de dominio, eliminación de los errores de aplicación por comando, reemplazo de los *result assemblers* por comando con un único *action result assembler*, retorno de respuestas `ProblemDetails` (RFC 7807) y localización de mensajes (ES/EN) mediante recursos `.resx`. Sobre esa base transversal se añadió valor funcional: el flujo de recuperación de contraseña con servicio SMTP (IAM), la integración real de pagos con Stripe (Subscriptions & Billing), la sincronización automática con Google Health y el estimador de calorías activas (ActivityWearable) y el permiso de ubicación para recomendaciones (SmartRecommendations). Tras el release `v2.0.0`, la integración con Google Health en producción expuso incidencias de OAuth (alcance de permisos, URI de redirección y tipo de dato sincronizado) que se corrigieron mediante tres hotfixes sucesivos (`v1.6.1`, `v1.6.2` y `v1.6.3`). Ya sobre `main`, el backend recibió dos hotfixes adicionales —`v2.0.1`, que corrige la *base URL* de producción, y `v2.0.2`, que ajusta el *user resource*— y culminó con el release funcional `v2.1.0`, que actualiza la integración con DeepSeek y amplía el *seed* de la categoría de legumbres. En paralelo, la Web Application (`nutrisense-webapp`) incorporó el flujo de recuperación de contraseña, la conexión OAuth con Google Health y la detección automática de ciudad de origen para las recomendaciones sensibles a la ubicación, liberados como `v2.2.0` con un hotfix `v2.2.1` para la configuración de producción. Posteriormente, el frontend evolucionó hasta el release `v3.0.0`, que incorpora la captura por cámara para el escaneo de platos y menús (NutritionTracking), el *welcome banner* y el CTA de racha en el dashboard (AnalyticsReporting), la adaptación responsive y la localización de los formularios de perfil y onboarding (IAM), la localización del elemento de tarjeta de Stripe (Subscriptions & Billing) y diversos ajustes de *UI polish*. La Landing Page (`nutrisense-website`) no recibió cambios de implementación en esta iteración.

A continuación se presenta la relación de commits asociados a la implementación del sprint.

| Repository | Branch | Commit Id | Commit Message | Commit Message Body | Committed on |
|---|---|---|---|---|---|
| nutrisense-platform | main | `e6c2c48` | Merge pull request #44 from upc-pre-202610-1asi0730-12053-nutrisens/develop | Release/2.1.0 | 2026-07-04 |
| nutrisense-platform | main | `600d68a` | Merge pull request #43 from upc-pre-202610-1asi0730-12053-nutrisens/feature/legume-category-seed-expansion | Feature/legume category seed expansion | 2026-07-04 |
| nutrisense-platform | main | `fbe7286` | chore: bump version to 2.1.0 | — | 2026-07-04 |
| nutrisense-platform | main | `36ecb8c` | chore: update deepseek integration | — | 2026-07-04 |
| nutrisense-platform | main | `ca2f279` | chore: update appsettings | — | 2026-07-04 |
| nutrisense-platform | main | `5b3052a` | Merge pull request #42 from upc-pre-202610-1asi0730-12053-nutrisens/hotfix/2.0.2-user-resource | Hotfix/2.0.2 user resource | 2026-07-04 |
| nutrisense-platform | main | `bcff84b` | Merge pull request #41 from upc-pre-202610-1asi0730-12053-nutrisens/hotfix/2.0.2-user-resource | chore: update user resource | 2026-07-04 |
| nutrisense-platform | main | `78758e3` | chore: bump version to 2.0.2 | — | 2026-07-04 |
| nutrisense-platform | main | `8ffbdb8` | chore: update user resource | — | 2026-07-04 |
| nutrisense-platform | main | `45ca6ce` | Merge pull request #40 from upc-pre-202610-1asi0730-12053-nutrisens/develop | Release/2.0.1 | 2026-07-04 |
| nutrisense-platform | main | `90c69a4` | Merge pull request #39 from upc-pre-202610-1asi0730-12053-nutrisens/hotfix/2.0.1-base-url | Hotfix/2.0.1 base url | 2026-07-04 |
| nutrisense-platform | main | `acc5786` | chore: update README | — | 2026-07-04 |
| nutrisense-platform | main | `bc55459` | chore: bump version to 2.0.1 | — | 2026-07-04 |
| nutrisense-platform | main | `4fa4031` | chore: update appsettings | — | 2026-07-04 |
...
| nutrisense-webapp | main | `296ff4b` | Merge pull request #31 from upc-pre-202610-1asi0730-12053-nutrisens/develop | Release/3.0.0 | 2026-07-05 |
| nutrisense-webapp | main | `6d19c97` | Merge pull request #30 from upc-pre-202610-1asi0730-12053-nutrisens/fix/ui-polish | Fix/UI polish | 2026-07-04 |
| nutrisense-webapp | main | `273b39e` | fix(shared): fix mobile overflow in activity form, toast, and recommendation labels | Prevents the activity type grid, intensity selector, and form rows from overflowing on narrow screens; keeps the toast width within small viewports; uses distinct translated labels for the macro filter chips instead of reusing generic macro names; and corrects the Spanish TDEE abbreviation. | 2026-07-04 |
| nutrisense-webapp | main | `64ddb5d` | fix(subscriptions-billing): localize Stripe card element and drop card deletion | Passes the active app locale into Stripe Elements so its own placeholders and error messages match the UI language, replaces hardcoded English strings on the checkout form with translated ones, and removes the payment-method deletion action from the billing profile page. | 2026-07-04 |
| nutrisense-webapp | main | `12fa0e0` | fix(iam): make profile and onboarding forms responsive, localize delete confirmation | Fixes profile nav, sex selector, and personal-info form overflowing on narrow viewports; makes height read-only after onboarding; localizes the account-deletion confirmation keyword instead of hardcoding "DELETE"; adds the terms-and-conditions link to registration and onboarding; and passes dietary restrictions directly in the profile update payload instead of a separate follow-up call. | 2026-07-04 |
| nutrisense-webapp | main | `e89fd6b` | feat(analytics-reporting): add welcome banner and streak CTA to dashboard | Guides new or inactive users (no meals logged, no streak, no completed days this week) toward logging their first meal instead of showing bare zeroed-out numbers, and turns the broken-streak pill into a clickable CTA. | 2026-07-04 |
| nutrisense-webapp | main | `96b5053` | feat(nutrition-tracking): add camera capture for dish and menu scanning | Adds a shared camera-capture component using getUserMedia so "Take photo" activates the device camera instead of falling back to the OS file picker, and blocks the sibling scan card while a scan is in flight to avoid races. | 2026-07-04 |
...
Adicionalmente, se incorporaron a la especificación los **nuevos endpoints** derivados de las funcionalidades del sprint: el flujo de recuperación de contraseña (IAM), la preferencia de auto-sincronización con Google Health (ActivityWearable) y el permiso de ubicación (SmartRecommendations). Con estas incorporaciones, la plataforma pasó de 74 a **78 endpoints REST** documentados bajo `/api/v1/`. Los releases posteriores sobre `main` (`v2.0.1`, `v2.0.2` y `v2.1.0`) no alteraron la superficie de endpoints: se limitaron a corregir la *base URL* de producción, ajustar el esquema del *user resource* devuelto por el contexto IAM y actualizar la integración interna con DeepSeek junto con la ampliación del *seed* de la categoría de legumbres, cambios que se reflejan en la especificación OpenAPI sin sumar nuevas rutas.
...
El resto de los 78 endpoints documentados en el Sprint 3 se mantiene sin cambios de contrato de éxito; la única modificación transversal es la adopción del formato `ProblemDetails` localizado en todas sus respuestas de error. La relación completa por bounded context se detalla en la sección **5.2.3.6** de este documento.

#### 5.2.4.7. Software Deployment Evidence for Sprint Review

Durante el Sprint 4 no se crearon nuevos productos ni infraestructura; el despliegue consistió en la **actualización del Web Service** (`nutrisense-platform`) y de la **Web Application** (`nutrisense-webapp`), ambos ya desplegados desde sprints anteriores, promoviendo a producción el release `v2.1.0` del backend (con la estandarización de errores, las nuevas integraciones externas, tres hotfixes de OAuth con Google Health, dos hotfixes adicionales sobre `main` —`v2.0.1` de *base URL* y `v2.0.2` de *user resource*— y la actualización de la integración con DeepSeek junto con la ampliación del *seed* de legumbres) y el release `v3.0.0` del frontend (con la recuperación de contraseña, la conexión OAuth con Google Health, la detección automática de ubicación, la captura por cámara, el *welcome banner* con CTA de racha y los ajustes responsive, de localización y de *UI polish*), junto con el hotfix `v2.2.1` sobre la configuración de producción del frontend. La Landing Page se mantuvo sin cambios de despliegue. A continuación se describen los pasos realizados.

##### Merge a main y creación de los tags de release `v2.0.0` y `v2.2.0`

Una vez integradas todas las ramas `feature/*` del sprint hacia `develop` (release intermedio `v1.6.0` en el backend), se realizó el merge de `develop` a `main` mediante Pull Requests en GitHub (PR #37/#38 en `nutrisense-platform`, commits `cbc049f` y `75f3f3c`), etiquetando el commit resultante como `v2.0.0`. En producción, la integración con Google Health expuso incidencias de OAuth que se resolvieron con los hotfixes `v1.6.1`, `v1.6.2` y `v1.6.3`, cada uno mergeado directamente a `main`. Posteriormente, sobre `main` se aplicaron los hotfixes `v2.0.1` (PR #39/#40, *base URL*) y `v2.0.2` (PR #41/#42, *user resource*) y se publicó el release funcional `v2.1.0` (PR #43/#44, commit `e6c2c48`) con la actualización de la integración con DeepSeek y la ampliación del *seed* de la categoría de legumbres. De forma análoga, en `nutrisense-webapp` el merge de `develop` a `main` (PR #27, commit `ba26b3a`) publicó el release `v2.2.0`, seguido del hotfix `v2.2.1` (commit `0ccd04a`) para la configuración de variables de entorno de producción y, finalmente, del release `v3.0.0` (PR #31, commit `296ff4b`) que suma la captura por cámara, el *welcome banner* con CTA de racha en el dashboard y los ajustes responsive, de localización y de *UI polish*. Cada merge a `main` disparó automáticamente el redespliegue continuo en Coolify del servicio correspondiente.
